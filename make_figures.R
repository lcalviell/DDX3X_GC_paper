...
a<-a + theme_bw() +
    ...
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=18)) +
    theme(axis.title.y = element_text(size=24),axis.text.y  = element_text(angle=45, vjust=0.5, size=18)) 
# gives error since Oct 2023: scale_alpha_continuous("# mRNAs",trans = scales::trans_new(name = "test",transform =invix,inverse = xivni),range = c(0.2,1),breaks=c(5,10,50))
...
meth<-sapply(strsplit(colnames(enc_dfres[[1]]),"_"),"[[",3)

kds<-c()
pvs<-c()
n_sigi<-c()

for(i in 1:length(enc_dfres[[1]][1,])){
    
    kds<-c(kds,enc_dfres$log2FC[ginon==arbipi[i],i])
    pvs<-c(pvs,enc_dfres$padj[ginon==arbipi[i],i])
    n_sigi<-c(n_sigi,length(which(enc_dfres$padj[,i]<.01)))
}

kdos<-kds


names(kdos)<-colnames(enc_dfres$log2FC)
...
pdf(file = "Figures/Fig3_supp_ENC_rf_overview.pdf",width = 32,height = 5) 
...
dfrev<-df
dfrev$label<-NA
dfrev<-dfrev[dfrev$cell=="K562" & dfrev$method=="shRNA",]
oka<-which(dfrev$corrp_tst>.7 & dfrev$importance>.0325)
dfrev$label[oka]<-dfrev$RBP[oka]

dfrev$label[dfrev$Var1!="GCpct_cds"]<-NA
dfrev$label[dfrev$RBP=="DDX3X" & dfrev$Var1=="GCpct_cds"]<-"DDX3X"

a<-ggplot(dfrev,aes(x=importance,y=corrp_tst,color=Var1,label=label)) + geom_smooth(se = F) + geom_point(size=.9)
a<-a + theme_classic() +
    xlab("Feature importance") +
    ylab("Method performance") +
    theme(axis.title.x = element_text(size=24),axis.text.x  = element_text(angle=45, vjust=0.5, size=18)) +
    theme(axis.title.y = element_text(size=24),axis.text.y  = element_text(angle=45, vjust=0.5, size=18))  +
    scale_color_manual(values = c("dark grey","dark blue","steelblue"),"Feature")

pdf(file = "Figures/Fig4C.2.pdf",width = 10,height = 6)
a + geom_text_repel(show.legend = F, max.overlaps=30)
dev.off()
...
pdf(file = "Figures/Fig4C_2_supp.pdf",width = 10,height = 6)
a + geom_text_repel(show.legend = F, max.overlaps=30) + facet_wrap(~experiment) 
dev.off()
...
pl + scale_color_manual(values = colss,"Time after\ndegron activation")+ geom_vline(xintercept = stpnts,color=c(colss),linetype=2) + geom_point(data = datap,size=5,color=c(colss))
dev.off()
...
qc2<-qc2[,c(5,1,2,3,4)]
write.table(qc2,file = "Tables/Supplementary_Table_1.tsv",sep="\t",quote = F,row.names = F)
...
ggcc<-ggplot(df, aes(x=Time, y=value, group=phtr, color=Phase,alpha=Treatment,shape=Treatment)) + 
    stat_summary(geom="line",size=1.2) +
    ...
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) +
    ylab("% of total cells") +
    xlab("Hours") +  scale_color_manual(values = c("orange"," dark red","dark blue"),"Cell cycle phase") +
    scale_alpha_manual(values=c("DMSO"=.5,"Auxin"=1)) + scale_x_continuous(breaks = c(0,4,8,16,24,48),labels = as.character(c(0,4,8,16,24,48)))


pdf(file = "Figures/Fig4F_supp3.pdf",width = 9,height = 4)
ggcc 
dev.off() 

df <- read.csv("data/table_021822_ok.csv",sep = ",",header = F)
colnames(df)<-c("name","S","G2","G1")
df<-rbind(df[1:3,],df)
df[1:3,1]<-"0hr_Auxin"
df<-melt(df)
...
pdf(file = "Figures/Fig4F_supp2.pdf",width = 9,height = 4)
ggcc 
dev.off()

df1 <- read.csv("data/table_031122_ok.tsv",sep = "\t",header = F)
colnames(df1)<-c("name","S","G2","G1")
df2 <- read.csv("data/table_021822_ok.csv",sep = ",",header = F)
colnames(df2)<-c("name","S","G2","G1")
df1$replicate<-1
df2$replicate<-2

df1$Treatment<-"DMSO"
df1$Treatment[grep(df1$name,pattern = "Auxin")]<-"Auxin"
df1$Time<-gsub(sapply(strsplit(df1$name,"_"),"[[",1),pattern = "hr",replacement = "")
df2$Treatment<-"DMSO"
df2$Treatment[grep(df2$name,pattern = "Auxin")]<-"Auxin"
df2$Time<-gsub(sapply(strsplit(df2$name,"_"),"[[",1),pattern = "hr",replacement = "")
df<-rbind(df1,df2)
df<-lapply(split(df,f = df$replicate),function(x){
    x<-lapply(split(x,x$Time),function(y){
        colmi<-colMeans(y[1:3,2:4])
        y[,2]<-y[,2]/colmi[1]
        y[,3]<-y[,3]/colmi[2]
        y[,4]<-y[,4]/colmi[3]
    ...
    x<-do.call(x,what = rbind)
    x
})
df<-do.call(df,what = rbind)
df$replicate<-NULL
df<-melt(df)
df$Treatment<-factor(df$Treatment,levels = c("DMSO","Auxin"))
df$Time<-as.numeric(df$Time)
df$Time<-as.factor(as.numeric(df$Time))

cellnorm<-ggplot(df,aes(y = value,x=Time,color=Treatment)) + geom_jitter(position = position_dodge(width = .9),size=.3)+ stat_summary(position =  position_dodge(width = .9),size=1.2)+
    xlab("hours past degron induction") +
    ylab("Cell proportion\nnormalized to DMSO") +
    scale_color_manual(values = c("dark grey","black"),"Treatment") +
    theme_classic() +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=13)) +
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) +
    stat_compare_means(method = "wilcox.test",na.rm = T,label="p.signif",hide.ns = T,paired = F)


pdf(file = "Figures/Fig4F_supp.pdf",width = 9,height = 4)
cellnorm + facet_wrap(~variable) + geom_hline(yintercept = 1,lty=2)
dev.off()
...
df<-get(load("data/slam_qc.RData"))
df2<-df[grep(as.character(df$sample),pattern = "IC85|NT"),]
g2 <- ggplot(data = df2, aes(x=mutation, y=value, fill=mutation)) + 
    geom_bar(stat="identity") + 
    labs(x="", y="read with mutation\nper million") +
    scale_fill_brewer(palette = "Set2") +
    theme_minimal() +
    theme(plot.margin = ggplot2::margin(.5, .5, .5, .5, "cm"),
          ...
          panel.grid.major.x = element_line( size=.1, color="dark grey" ,linetype=2)) 

pdf(file = "Figures/Supplementary_Figure_2.pdf",width = 9,height = 10)
g2 + facet_wrap(~sample,ncol = 2)
dev.off()

dfok<-get(load("data/SLAM_DE.RData"))

colsi<-alpha(c("blue","dark red"),c(.8,.8))
names(colsi)<-c("TE_down","TE_up")

plot_deg<-ggplot(dfok,aes(y=value,x=time,alpha=condition,shape=condition,color=tx_type)) + 
    stat_summary(geom="line",size=1.2) +
    ylab("% T>C reads") +
    xlab("") +
    ...
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white"))+
    scale_x_continuous(breaks=c(-24,-12,0,8), labels=c("t0 labeling","24h labeling","chase\n+IAA","IAA\n8h")) +
    geom_vline(xintercept = 0,lty=2)

pdf(file = "Figures/Fig2b.pdf",width = 9,height = 5)
plot_deg + stat_summary(size=1.2) + scale_alpha_manual(values=c("DMSO"=.3,"Degron"=1))
dev.off()



plot_deg2<-ggplot(dfok,aes(y=value,x=time,alpha=condition,shape=condition,color=tx_type_gc)) + 
    stat_summary(geom="line",size=1.2) +
    ylab("% T>C reads") +
    xlab("") +
    scale_color_manual(values = alpha(c("forestgreen","gray","purple"),.8),"%GC in CDS") +
    theme_classic() +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=13)) +
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white"))+
...
pdf(file = "Figures/Fig3F.pdf",width = 9,height = 5)
plot_deg2 + stat_summary(size=1.2) + scale_alpha_manual(values=c("DMSO"=.3,"Degron"=1)) 
dev.off()


df8<-dfok[dfok$time==8,]


df8_ted<-df8[df8$tx_type=="TE_down",]
df8_teu<-df8[df8$tx_type=="TE_up",]

dfted<-melt(split(df8_ted$value,df8_ted$condition))
dfted$tx_type<-"TE_down"
dfteu<-melt(split(df8_teu$value,df8_teu$condition))
dfteu$tx_type<-"TE_up"
dfte<-rbind(dfted,dfteu)

pdf(file = "Figures/Fig2b_supp.pdf",width = 9,height = 5)
dfte$L1<-factor(dfte$L1,levels = c("DMSO","Degron"))
plot_slamte<-ggplot(dfte,aes(y=value,x=L1,color=L1,group=L1)) + geom_boxplot(position = position_dodge())+
    geom_jitter(alpha=.3,size=.2) +
    ...
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) +
    stat_compare_means(method = "wilcox.test",label.y = 100 + seq(0,1,length.out = 5),method.args =list(alternative="l"))
plot_slamte + facet_wrap(~tx_type,nrow = 1)
...
plot_slamte<-ggplot(dfte,aes(y=value,x=L1,color=L1,group=L1)) + geom_boxplot(position = position_dodge())+
    ...
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) +
    stat_compare_means(method = "wilcox.test",label.y = 100 + seq(0,1,length.out = 5),method.args =list(alternative="g"))
plot_slamte + facet_wrap(~tx_type,nrow = 1)


df8_lowgc<-df8[df8$tx_type_gc=="38-51.8",]
df8_midgc<-df8[df8$tx_type_gc=="51.8-65.7",]
df8_highgc<-df8[df8$tx_type_gc=="65.7-79.6",]

df8_lowgc<-melt(split(df8_lowgc$value,df8_lowgc$condition))
df8_lowgc$tx_type<-"38-51.8"
df8_midgc<-melt(split(df8_midgc$value,df8_midgc$condition))
df8_midgc$tx_type<-"51.8-65.7"
df8_highgc<-melt(split(df8_highgc$value,df8_highgc$condition))
df8_highgc$tx_type<-"65.7-79.6"

dfte<-rbind(df8_lowgc,df8_midgc,df8_highgc)
dfte$L1<-factor(dfte$L1,levels = c("DMSO","Degron"))

dev.off()

pdf(file = "Figures/Fig3f_supp.pdf",width = 9,height = 5)
...
dfok$Var2<-paste(dfok$condition,dfok$time,sep = "_")
dats<-split(dfok$value,interaction(dfok$Var2,dfok$tx_type))
list_dats<-c()
nims<-names(dats)
typs<-unique(as.character(dfok$tx_type))
for(i in c("Degron","DMSO")){
    list_dat<-list()
    for(j in typs){
        nimsok<-nims[grepl(nims,pattern = j) & grepl(nims,pattern = i)]
        list_dat[[j]]<-dats[[nimsok[grep(nimsok,pattern = "_8")]]]/dats[[nimsok[grep(nimsok,pattern = "_0")]]]
    }
    xx<-melt(list_dat)
    xx$condition<-i
    list_dats[[i]]<-xx
}
list_dats_TE<-do.call(list_dats,what = rbind)


dats<-split(dfok$value,interaction(dfok$Var2,dfok$tx_type_gc))
list_dats<-c()
nims<-names(dats)
typs<-unique(as.character(dfok$tx_type_gc))
for(i in c("Degron","DMSO")){
    list_dat<-list()
    for(j in typs){
        nimsok<-nims[grepl(nims,pattern = j) & grepl(nims,pattern = i)]
...
list_dats_gc<-do.call(list_dats,what = rbind)
...
df<-as.data.frame(goexample)
df$Term<-factor(df$Term,levels=rev(as.character(df$Term)))

AAA<-ggplot(data = df, aes(x=classicFisher,y=Term)) + geom_bar(stat = "identity") + theme_bw() +
    xlab("Adj. p-value") +
    ylab("") +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=18)) +
    theme(axis.title.y = element_text(size=24),axis.text.y  = element_text(angle=0, vjust=0.5, size=10)) +
    scale_x_log10()

pdf(file = "Figures/GO.rev.pdf",width = 10,height = 5)
AAA + geom_vline(xintercept = .05,color="red")
dev.off()


list_dfs<-get(load("data/degron_multiDE_results_summary.RData"))

df<-list_dfs$uniq$`48h`
...
df[,colnames(df)[grep(colnames(df),pattern = "TPM")]]<-log2(df[,colnames(df)[grep(colnames(df),pattern = "TPM")]])
df<-df[df$TPM_RNA>3,]
df<-df[,grep(colnames(df),pattern = "GC|codo|RiboRNA_log2FC")]

corre<-c()
for(i in colnames(df)[colnames(df)!="RiboRNA_log2FC"]){
    corre<-c(corre,cor.test(df[,i],df[,"RiboRNA_log2FC"])$estimate)
}
names(corre)<-colnames(df)[colnames(df)!="RiboRNA_log2FC"]
names(corre)<-gsub(names(corre),pattern = "codonfr_",replacement = "")
corre<-sort(corre,decreasing = F)
df<-melt(corre)
df$variable<-rownames(df)
df$variable<-factor(df$variable,levels=as.character(df$variable))
df$gcrich<-"other"

df$gcrich[grep(df$variable,pattern = "GGC|GCC|CCC|GGG|CGG|CGC|CCG|GCG|gc|GCpc")]<-"High-GC"
df$gcrich[grep(df$variable,pattern = "AAA|UUU|AUA|AUU|UAA|UAU|AAU|UUA")]<-"Low-GC"

df$gcrich<-factor(df$gcrich,levels = c("High-GC","other","Low-GC"))
AAA<-ggplot(data = df, aes(x=variable,y=value,fill=gcrich)) + geom_bar(stat = "identity") + theme_bw() +
    xlab("") +
    ylab("Pearson correlation\nto TE changes") +
    theme(axis.title.x = element_text(size=20),axis.text.x  = element_text(angle=90, vjust=0.5, size=10)) +
    theme(axis.title.y = element_text(size=20),axis.text.y  = element_text(angle=0, vjust=0.5, size=10)) +
    scale_fill_manual(values = c("blue","dark grey","red"),"")

pdf(file = "Figures/correl_GC.rev.pdf",width = 14,height = 5)
AAA + geom_vline(xintercept = .05,color="red")
dev.off()
...
pdf(file = "Figures/Cell_cycle1.rev.pdf",width = 9,height = 4)
ggcc 
dev.off()
...
df$Time<-as.numeric(df$Time)
df$Time<-as.factor(as.numeric(df$Time))
df$variable<-factor(df$variable,levels=c("G1","S","G2"))
df<-df[df$Time!=0,]
cellnorm<-ggplot(df,aes(y = value,x=Time,color=Treatment)) + geom_jitter(position = position_dodge(width = .9),size=.3)+ stat_summary(position =  position_dodge(width = .9),size=1.1)+
    xlab("hours past degron induction") +
    ylab("Cell proportion\nnormalized to DMSO") +
    scale_color_manual(values = c("dark grey","black"),"Treatment") +
    theme_classic() +
...
pdf(file = "Figures/Cell_cycle2.rev.pdf",width = 10,height = 5)
cellnorm + facet_wrap(~variable) + geom_hline(yintercept = 1,lty=2)
dev.off()

df<-get(load("data/gc_startend_rf_100200_AA.RData"))
rfone2<-df
nims2<-names(rfone2)
ress_rfimp<-list()
for(i in nims2){
...
aa<-do.call(ress_rfimp,what = rbind)
aa$Var1<-as.character(aa$Var1)
aa$Var1[grep(aa$Var1,pattern = "_2$")]<-gsub(aa$Var1[grep(aa$Var1,pattern = "_2$")],pattern = "_2",replacement = "_end")
aa$Var1[grep(aa$Var1,pattern = "_1$")]<-gsub(aa$Var1[grep(aa$Var1,pattern = "_1$")],pattern = "_1",replacement = "_start")
aa$Var1[grep(aa$Var1,pattern = "_end00")]<-gsub(aa$Var1[grep(aa$Var1,pattern = "_end00")],pattern = "_end00",replacement = "_200")
aa$Var1[grep(aa$Var1,pattern = "_start00")]<-gsub(aa$Var1[grep(aa$Var1,pattern = "_start00")],pattern = "_start00",replacement = "_100")


aa$Var1<-factor(aa$Var1,levels=unique(aa$Var1))
aa<-aa[aa$feature=="delta_TE",]
vars<-unique(aa$Var1)
...
rf_others<-levels(new_ress_rfimp$Var1)[1:85]
rf_others_amino<-levels(new_ress_rfimp$Var1)[grep(levels(new_ress_rfimp$Var1),pattern = "amino")]
new_ress_rfimp$Var1<-as.character(new_ress_rfimp$Var1)
new_ress_rfimp$Var1[new_ress_rfimp$Var1%in%rf_others_amino]<-"amino_freqs"
new_ress_rfimp$Var1[new_ress_rfimp$Var1%in%rf_others]<-"other"
new_ress_rfimp<-aggregate(new_ress_rfimp$value,list(new_ress_rfimp$Var1,new_ress_rfimp$L1,new_ress_rfimp$xy,new_ress_rfimp$feature),mean)
colnames(new_ress_rfimp)<-c("Var1","L1","xy","feature","value")
topsi<-rev(as.character(unique(new_ress_rfimp$Var1[order(new_ress_rfimp$value,decreasing = T)])))
...
pdf(file = "Figures/RF_AA_100200.rev.pdf",width = 6,height = 8)
rf_impor
dev.off()
...
dfteu$tx_type<-"TE_up"
dfte<-rbind(dfted,dfteu)
pdf(file = "Figures/SlamTE_Fig2b_supp.rev.pdf",width = 9,height = 5)
dfte$L1<-factor(dfte$L1,levels = c("DMSO","Degron"))
plot_slamte<-ggplot(dfte,aes(y=value,x=L1,color=L1,group=L1)) + geom_boxplot(position = position_dodge())+
    geom_jitter(alpha=.3,size=.2) +
    scale_color_manual(values=c("dark grey","black"),"")+
...
pdf(file = "Figures/Fig3f_slamTE_supp.rev.pdf",width = 9,height = 5)
...
pdf(file = "Figures/Fig3_supp_ENC_rf_overview.rev.pdf",width = 20,height = 5)
a+facet_wrap(feature~.,nrow = 2)
a
dev.off()
...
pdf(file = "Figures/Fig3_supp_ENC_rf_overview2.rev.pdf",width = 50,height = 9)
a+facet_wrap(feature~.,nrow = 2)
a+facet_wrap(type_exp~.,nrow = length(unique(df2$type_exp)),drop = T)
dev.off()
...
pdf(file = "Figures/Fig4C.rev.pdf",width = 10,height = 6)
a + geom_text_repel(show.legend = F)
dev.off()
...
pdf(file = "Figures/Fig4C.rev.2.pdf",width = 10,height = 6)
a + geom_text_repel(show.legend = F, max.overlaps=30) 
dev.off()
...
pdf(file = "Figures/Fig4C_new_supp.rev.pdf",width = 10,height = 6)
a + geom_text_repel(show.legend = F, max.overlaps=30) + facet_wrap(~experiment)
dev.off()


icsh_min<-import(con = "data/icSHAPE_hg38_Nature_Structural___Molecular_Biology_2019_NAI-N3_cytoplasm_invivo_minus_score_cytoplasm_vivoNAIN3-score-neg.bw")
icsh_pl<-import(con = "data/icSHAPE_hg38_Nature_Structural___Molecular_Biology_2019_NAI-N3_cytoplasm_invivo_plus_score_cytoplasm_vivoNAIN3-score-pos.bw")
strand(icsh_min)<-"-"
strand(icsh_pl)<-"+"
icsh_min<-coverage(icsh_min,weight = icsh_min$score)
...
dfok<-dfok[okko,]
dfok$tx_type_RiboRNA<-"unchanged"

dfok$tx_type_RiboRNA[dfok$RNA_padj<.01 & dfok$RNA_log2FC<0 & dfok$Ribo_log2FC<0]<-"Concordant_down"
dfok$tx_type_RiboRNA[dfok$RNA_padj<.01 & dfok$RNA_log2FC>0 & dfok$Ribo_log2FC>0]<-"Concordant_up"

dfok$tx_type_RiboRNA[dfok$RiboRNA_padj<.05 & dfok$RiboRNA_log2FC<0]<-"TE_down"
dfok$tx_type_RiboRNA[dfok$RiboRNA_padj<.05 & dfok$RiboRNA_log2FC>0]<-"TE_up"

dfok$tx_type_RiboRNAxy<-"unchanged"

min_xy<-sort(abs(dfok$totdist_RiboRNA),decreasing = T)[1:401][401]
dfok$tx_type_RiboRNAxy[which(abs(dfok$totdist_RiboRNA)>min_xy)]<-dfok$xy_RiboRNA[which(abs(dfok$totdist_RiboRNA)>min_xy)]

gcqnt<-setNames(cut(dfok$GCpct_cds*100,breaks = quantile(dfok$GCpct_cds*100,prob=c(seq(0,1,length.out = 4))),include.lowest = T,right = T),nm = dfok$gene_id)

gcqnt<-gsub(gsub(gsub(gsub(as.character(gcqnt),pattern = "[(]",replacement = ""),pattern = "\\[",replacement = ""),pattern = "\\]",replacement = ""),pattern = ",",replacement = "-")
dfok$gcqnt<-gcqnt

regg<-unlist(regions$cds[dfok$gene_id])
ic_str<-icsh[regg]
ic_str<-sum(NumericList(split(sum(ic_str),names(regg))))
ic_str_norm<-ic_str/sum(NumericList(split(width(regg),names(regg))))
dfok$ic_str_norm<-ic_str_norm

df2<-dfok[dfok$ic_str_norm>0,]

list_str<-split(df2$ic_str_norm,df2$gcqnt)
list_str<-melt(list_str)

str_plot2<-ggplot(list_str,aes(y = value,x=L1,color=L1)) + geom_jitter(size=.2)+ geom_boxplot()+
    ylab("icSHAPE reactivity\nin CDS") +
    xlab("GCpct_cds") +
    scale_color_manual(values = alpha(c("forestgreen","dark grey","purple"),.8),"GC_cds") +
    theme_classic() +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=13)) +
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) +
    stat_compare_means(ref.group = names(table(list_str$L1)[2]),method = "wilcox.test",na.rm = T,label="p.signif",hide.ns = T,paired = F)

pdf(file = "Figures/icSHAPE.rev.pdf",width = 10,height = 6)
str_plot2+ theme(legend.position="none")
dev.off()


data_nar<-get(load("data/processed_data_NAR.rdata"))

maptx<-data_nar$peaks_tx
bin5<-15
bin_c<-30
bin_u<-20
...
agga2$xy_RiboRNA<-dfok$xy_RiboRNA[match(agga2$gene_name,dfok$gene_name)]
agga2$tx_type_new<-dfok$tx_type_RiboRNA[match(agga2$gene_name,dfok$gene_name)]



agga2<-agga2[!is.na(agga2$xy_RiboRNA),]
agga2$xy_RiboRNA<-factor(as.character(agga2$xy_RiboRNA),levels = c("-xy","x","y","xy"))
btps<-unique(agga2$tx_type_new)
comps<-unname(split(cbind(rep("unchanged",length(btps)),as.character(btps)),f = 1:(length(btps))))
comps<-comps[c(3,1,4,5)]
comps<-comps[c(1,3)]
agga3<-agga2
agga3<-agga3[agga3$tx_type_new%in%c("TE_up","TE_down","unchanged"),]
agga3<-agga3[agga3$tx_position%in%c("5_UTR","CDS"),]

conv_specplot<-ggplot(agga3,aes(y=conv_spec,x=tx_type_new,group=tx_type_new,color=tx_type_new)) + 
    ...
    scale_color_manual(values = c("blue","dark red","dark gray"),"Tx_class") + 
    geom_vline(xintercept =bin5+bin_c+1,col="black",lty=3)+ 
    theme_classic() +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=13)) +
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) 
conv_specplot<-conv_specplot + stat_compare_means(label="p.signif",method = "wilcox.test",hide.ns = F,comparisons = comps,label.y = .53 + seq(0,.14,length.out = 4)) + facet_wrap(.~tx_position,ncol = 4) 
pdf(file = "Figures/peaks_supp1.rev.pdf",width = 8,height = 4)
conv_specplot+ theme(axis.title.x=element_blank(),
                     axis.text.x=element_blank(),
                     axis.ticks.x=element_blank())
...
aggall<-agga3
aggall$newtxtype<-paste(aggall$tx_type_new,aggall$xy_RiboRNA,sep = ";")
aggall$newtxtype[aggall$tx_type_new=="unchanged"]<-"unchanged"
aggall<-aggall[grep(aggall$newtxtype,pattern = ";xy",invert = T),]
aggall$newtxtype<-factor(as.character(aggall$newtxtype),levels = c("TE_down;y","TE_down;-xy","TE_down;x","TE_up;y","TE_up;-xy","TE_up;x","unchanged"))
btps<-levels(aggall$newtxtype)
comps<-unname(split(cbind(rep("unchanged",length(btps)),as.character(btps)),f = 1:(length(btps))))
comps<-rev(comps[-7])

conv_specplot2<-ggplot(aggall,aes(y=conv_spec,x=newtxtype,group=newtxtype,color=newtxtype)) + 
    geom_jitter(alpha=.06) + 
    ...
    xlab("") +
    geom_vline(xintercept =bin5+1,col="black",lty=3)+
    scale_color_manual(values = c("dark blue","blue","cornflowerblue","dark red","red","orange","dark gray"),"Tx_class") + 
    geom_vline(xintercept =bin5+bin_c+1,col="black",lty=3)+ 
    theme_classic() +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=13)) +
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) 
conv_specplot2<-conv_specplot2 + stat_compare_means(label="p.signif",method = "wilcox.test",hide.ns = F,comparisons = comps,label.y = .3 + seq(0,.3,length.out=7))+ facet_wrap(.~tx_position,nrow = 1) 
pdf(file = "Figures/peaks_supp2.rev.pdf",width = 10,height = 4)
conv_specplot2+ theme(axis.title.x=element_blank(),
                      axis.text.x=element_blank(),
                      axis.ticks.x=element_blank())
dev.off()


load("data/QCmetrics_frames.RData")
dff<-melt(frames)
dff$value<-dff$value*100
dff$Var2<-dff$Var2-1
dff$Var2<-factor(dff$Var2)
fr_plot<-ggplot(dff,aes(y = value,x=Var2,color=Var2,fill=Var2)) + geom_jitter(size=1)+ stat_summary(geom = "bar")+
    ylab("% of reads\n29nt") +
    xlab("Frame") +
    scale_color_manual(values = alpha(c("dark red","cornflowerblue","gold"),.8),"") +
    scale_fill_manual(values = alpha(c("dark red","cornflowerblue","gold"),.8),"") +
    theme_classic() +
    theme(axis.title.x = element_text(size=22),axis.text.x  = element_text(angle=45, vjust=0.5, size=13)) +
    theme(axis.title.y = element_text(size=18),axis.text.y  = element_text(angle=0, vjust=0.5, size=15))  +
    theme(strip.text.x = element_text(size=10, face="bold"),strip.text.y = element_text(size=24),strip.background = element_rect(colour="black", fill="white")) + theme(legend.position = "none")

pdf(file = "Figures/frames_supp2.rev.pdf",width = 7,height = 4)
fr_plot
dev.off()

mutwt<-data_nar$DE_degron

dfok$mut_RNAlog2FC<-mutwt$log2FoldChange_RNA[match(dfok$gene_name,mutwt$gene_name)]
dfok$mut_Ribolog2FC<-mutwt$log2FoldChange_Ribo[match(dfok$gene_name,mutwt$gene_name)]

dfok$mut_deltaTE<-mutwt$log2FoldChange_RiboRNA[match(dfok$gene_name,mutwt$gene_name)]
dfgc<-unique(df[df$Var1=="GCpct_cds",c("RBP","importance","corrp_tst")])
rbps_gc<-unique(dfgc$RBP[dfgc$importance>.03 & dfgc$corrp_tst>.5])
...
for(ont in c("BP","MF","CC")){
    suppressMessages(GOdata <- new("topGOdata", ontology = ont, 
                                   allGenes = mapp,
                                   annot = annFUN.org, 
                                   mapping=go_package, 
                                   ID="symbol"))
    ...
    tmp$classicFisher[tmp$classicFisher=="< 1e-30"]<-min(as.numeric(tmp$classicFisher),na.rm = T)
    tmp$classicFisher<-as.numeric(tmp$classicFisher)
    tmp$genes_annotated<-CharacterList(genesInTerm(GOdata,whichGO = tmp$GO.ID))
    tmp$genes<-tmp$genes_annotated[tmp$genes_annotated%in%myG]
    tmp<-tmp[which(tmp$classicFisher<.10 & tmp$Annotated>1 & tmp$Significant>1),]
    res_go[[ont]]<-tmp
}
...
ress_rfimp<-ress_rfimp[ress_rfimp$feature%in%c("delta_TE","RNA_log2FC"),]
...
pdf(file = "Figures/RFrnafc.rev.pdf",width = 9,height = 5)
rf_impor
dev.off()
